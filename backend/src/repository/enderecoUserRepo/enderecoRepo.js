...
export const cadastrarEnderecoUser = async (info) => {
    const comando = `
    insert into endereco (nm_rua,num_casa,cep_casa,bairro,cidade,endereco_prin,usuario_id)
    values (?,?,?,?,?,?,?);
    `;
    const [resposta] = await connection.execute(comando,[info.nm_rua, info.num_casa, info.cep_casa, info.bairro, info.cidade, info.endereco_prin, info.usuario_id]);
